...
async fn test_get_full_url() {
    ...
    let short_url = create_body["short_url"]
        .as_str()
        .expect("short_url is a string");

    // Now, get the full URL
    let get_response = client.get(format!("/{}", short_url)).dispatch().await;
...
async fn test_get_full_url_not_found() {
    ...
    let response = client.get("/nonexistent123").dispatch().await;
...
async fn test_create_short_url_multiple_times() {
    ...
    assert_eq!(
        get_response1.headers().get_one("Location"),
        Some("https://example.com/unique")
    );
    assert_eq!(
        get_response2.headers().get_one("Location"),
        Some("https://example.com/unique")
    );
}
